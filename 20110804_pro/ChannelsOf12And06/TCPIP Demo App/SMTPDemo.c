...
//		    if((BUTTON2_IO == 0u) && (BUTTON3_IO == 0u))
		...
		case MAIL_BEGIN:
			if(SMTPBeginUsage())
			{
				...
				static BYTE RAMStringTo[] = "kekebey@126.com";
				...
				SMTPClient.Server.szROM = (ROM BYTE*)"mail.supcon.com";	// SMTP server address
				SMTPClient.ROMPointers.Server = 1;
				SMTPClient.Username.szROM = (ROM BYTE*)"jinweijiang";
				SMTPClient.ROMPointers.Username = 1;
				SMTPClient.Password.szROM = (ROM BYTE*)"11079406";
				SMTPClient.ROMPointers.Password = 1;
				SMTPClient.To.szRAM = RAMStringTo;
				SMTPClient.From.szROM = (ROM BYTE*)"\"SMTP Service\" <jinweijiang@supcon.com>";
