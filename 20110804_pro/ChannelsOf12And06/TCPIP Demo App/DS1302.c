...
void DS1302_Init(void)
{
	...
	RTC_SCL_TRIS = 0;
	RTC_SDA_TRIS = 1;
//	i = ReadDS1302( Add_RAM0 + 1 );
	if(BUTTON0_IO == 0u)
	{
		WriteDS1302( Add_CONTROL,0x00 );		//关闭写保护
		WriteDS1302( Add_MIN,0x16 );			//预置分钟时间
		WriteDS1302( Add_HR,0x17 );				//预置小时，24小时制
		WriteDS1302( Add_DATE,0x24 );			//预置日期
		WriteDS1302( Add_MONTH,0x12 );			//预置月份
		WriteDS1302( Add_DAY,0x05 );			//预置星期
		WriteDS1302( Add_YEAR,0x12 );			//预置年份
